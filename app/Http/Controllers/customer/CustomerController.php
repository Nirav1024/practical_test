...
class CustomerController extends Controller
{
    ...
    public function register_do(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $user->save();
            return redirect()->route('customer.login')->with('success', 'Your Registration successfully');
        }
    }
        ...
        if ($validator->fails()) {
            ...
            if (Auth::attempt(['email' => $request->email, 'password' => $request->password])) {
                if (auth()->user()->role == 'customer') {
                    ...
                    return redirect()->route('customer.login')->with('danger', 'You are not allowed to login from here');;
                }
            } else {
                return redirect()->route('customer.login')->with('danger', 'Please enter valid input');;
            }
        }
    }
}
